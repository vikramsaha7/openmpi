...
int main ( int argc, char *argv[] )
{
	...
	if(id==0){    // take polling input from node 0 only
		while(1){
			if(poll(&mypoll,1,100)){   // poll with timeout of 100 ms //if in 100 s no input than 0 is returned;
	...
	if(id!=0)			//to send each node's amount to node 0 for summing up and printing
	{
	...
	if(id==0)
	{
		...
		while(1)
		{
			...
			totmoney +=d;			//adding the money recieved from other nodes
		}
